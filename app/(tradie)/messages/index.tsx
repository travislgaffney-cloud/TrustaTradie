  ...
  return (
    <SafeAreaView style={[styles.safe, { backgroundColor: colors.background }]}>
      ...
      {loading ? (
        ...
        <EmptyState icon="💬" title="No messages yet" description="Chat with customers after submitting a quote." />
      ) : (
        <FlatList
          data={conversations}
          keyExtractor={(c) => c.id}
          ...
          renderItem={({ item }) => {
            const other = item.tradie_id === user?.id ? item.customer : item.tradie;
            return (
              <Pressable
                onPress={() => router.push(`/(tradie)/messages/${item.id}`)}
